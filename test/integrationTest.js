...
describe('Integration Test', () => {
    ...
    const createDEOS = (name, host, port, peers, removeData = true) => {
    ...
    step('start server 1', () => {
        return createDEOS(name1, 'localhost', 3001, [])
            .then((httpServer) => {
    ...
    step('start server 2', () => {
        return createDEOS(name2, 'localhost', 3002, [{ url: 'http://localhost:3001' }])
            .then((httpServer) => {
    ...
    describe('Complementary tests', () => {
        ...
        step('restart server 1', () => {
            return Promise.resolve()
                ...
                .then(() => {
                    return createDEOS(name1, 'localhost', 3001, [], false)
                        .then((httpServer) => {
                            context.httpServer1 = httpServer;
                        });
                });
        ...
        step('create a new transaction with more value than funds', (done) => {
            return Promise.resolve()
                .then(() => {
                    supertest(context.httpServer1.app)
                        .post(`/operator/wallets/${context.walletId}/transactions`)
                        .set({ password: walletPassword })
                        .send({
                            fromAddress: context.address1,
